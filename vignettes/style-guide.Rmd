...
The R Style Guide describes the rdev style conventions, along with automated checks ([styler](https://styler.r-lib.org) and
[lintr](https://lintr.r-lib.org)). The rdev R Style Guide incorporates both the
...
rdev uses an opinionated [lintr](https://lintr.r-lib.org) configuration (`.lintr`):
...
The extraction operator linter enforces use of `[[` over `$`. Since `$` is intended for interactive
use and supports partial mapping, it is safer to use `[[`. However, use of `$` is common practice,
including within the lintr code, and leads to more readable code.
...
is more succinct than an `if`/`else` block, `library()` and `require()`. Use of `require()` within
`.Rprofile` and `library()` within R Markdown documents is an appropriate and common practice,
although neither should be used within package code.
