...
R Analysis Packages are an extension of the standard R package layout
designed to support publishing a collection of R notebooks used for data
analysis as static html, typically using [GitHub
Pages](https://pages.github.com).

[R Notebooks](https://bookdown.org/yihui/rmarkdown/notebook.html)
include a YAML header and [R
Markdown](https://github.com/rstudio/rmarkdown) body. The Analysis
Package Layout follows the convention that the first non-blank line in
the body of an R notebook serves as a brief description of the work.
`rdev` includes an “Analysis Notebook” RStudio R Markdown template (File
\> New File \> Rmarkdown \> From Template) and a `README.Rmd` template
optionally installed with
[`use_analysis_package()`](https://jabenninghoff.github.io/rdev/reference/use_analysis_package.md)
that dynamically builds a list of notebooks and descriptions when
...
Notebooks using [Quarto](https://quarto.org) as an alternative to the
original
[`use_analysis_package()`](https://jabenninghoff.github.io/rdev/reference/use_analysis_package.md).

rdev versions 1.10.2 and newer also support use of Quarto Documents
(`.qmd`) for Analysis Notebooks, although RStudio currently doesn’t
support [`.qmd` files as document
...
| Path | R | g | Function |
|:---|:---|:---|:---|
| .Rbuildignore |  | `x` | Files excluded from the package, sorted using [`sort_rbuildignore()`](https://jabenninghoff.github.io/rdev/reference/sort_rbuildignore.md) |
...
| .gitattributes |  | `x` | Used to configure GitHub [language statistics](https://github.com/github-linguist/linguist) |
...
| .lintr |  | `x` | [lintr](https://lintr.r-lib.org/index.html) configuration as described in the Style Guide, [`vignette("style-guide")`](https://jabenninghoff.github.io/rdev/articles/style-guide.md) |
...
| LICENSE | `x` | `x` | Generated with [`usethis::use_mit_license()`](https://usethis.r-lib.org/reference/licenses.html) |
...
| NEWS.md | `x` | `x` | Release notes, used by [`get_release()`](https://jabenninghoff.github.io/rdev/reference/get_release.md), [pkgdown](https://pkgdown.r-lib.org) and [Quarto](https://quarto.org) |
| R | `x` | `x` | All project functions go here, with [roxygen2](https://roxygen2.r-lib.org) comments |
| README.Rmd |  | `x` | Generated with [`usethis::use_readme_md()`](https://usethis.r-lib.org/reference/use_readme_rmd.html) or rdev template |
| README.md | `x` | `x` | Generated with [`devtools::build_readme()`](https://devtools.r-lib.org/reference/build_rmd.html) |
...
| analysis | `x` | `x` | Exploratory data analysis in R Notebooks, R Presentations, and Quarto documents. Include functions from the R directory using [`library()`](https://rdrr.io/r/base/library.html), not [`source()`](https://rdrr.io/r/base/source.html). |
...
| docs |  | `x` | Used by [`build_analysis_site()`](https://jabenninghoff.github.io/rdev/reference/build_analysis_site.md) and [`build_quarto_site()`](https://jabenninghoff.github.io/rdev/reference/build_quarto_site.md) to render the R Package and Notebooks for GitHub Pages |
